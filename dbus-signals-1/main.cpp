...
int main(int argc, char *argv[])
{
 ...
 #if 0
    QDBusConnection::sessionBus().connect(networkManagerService,
                                          networkManagerPath,
                                          nmDeviceInterfaceEth,
                                          "StateChanged", recv, SLOT(receive(unsigned int)));

#else
    QDBusInterface *iface = new QDBusInterface(networkManagerService,
                                               networkManagerPath,
                                               nmDeviceInterfaceEth,
                                               QDBusConnection::systemBus(), recv);
    if(iface->isValid()) {
        iface->connect(iface, SIGNAL(StateChanged(unsigned int)), recv, SLOT(receive(unsigned int)));
    }
 #endif
